...
pub const KVCache = struct {
    ...
    pub fn init(allocator: std.mem.Allocator, n_layers: usize, max_seq_len: usize, n_kv_heads: usize, head_dim: usize) !KVCache {
...
pub const TransformerEngine = struct {
    ...
    pub fn init(allocator: std.mem.Allocator, config: ModelConfig, tok_embeddings: TensorRef, output_norm: []const f32, lm_head: TensorRef) !TransformerEngine {
        var cache = try KVCache.init(allocator, config.n_layers, config.max_seq_len, config.n_kv_heads, config.head_dim);
        ...
        const k = try allocator.alloc(f32, config.n_kv_heads * config.head_dim);
        errdefer allocator.free(k);
        const v = try allocator.alloc(f32, config.n_kv_heads * config.head_dim);
    ...
    pub fn initFromReader(allocator: std.mem.Allocator, reader: *reader_mod.HKReader) !*TransformerEngine {
        ...
        if (reader.metadata_map.getInt(metadata_mod.StandardKeys.ATTN_HEAD_COUNT)) |v| {
            if (v > 0) cfg.n_heads = @intCast(v);
        }
        if (reader.metadata_map.getInt(metadata_mod.StandardKeys.ATTN_HEAD_COUNT_KV)) |v| {
            if (v > 0) cfg.n_kv_heads = @intCast(v);
        } else {
            cfg.n_kv_heads = cfg.n_heads;
        }
        if (reader.metadata_map.getFloat(metadata_mod.StandardKeys.ATTN_LAYER_NORM_RMS_EPS)) |v| {
            if (v > 0.0) cfg.norm_eps = @floatCast(v);
        }
        if (reader.metadata_map.getFloat(metadata_mod.StandardKeys.ROPE_FREQ_BASE)) |v| {
    ...
    pub fn matVec(weight: TensorRef, in_x: []const f32, bias: ?[]const f32, out_y: []f32) void {
        switch (weight.storage_type) {
            .q8_0 => tensor_ops.gemvQ8_0(weight.data, in_x, bias, out_y, weight.rows, weight.cols),
            .q4_0 => tensor_ops.gemvQ4_0(weight.data, in_x, bias, out_y, weight.rows, weight.cols),
            .q4_k => tensor_ops.gemvQ4_K(weight.data, in_x, bias, out_y, weight.rows, weight.cols),
            .f32 => {
                const w_f32: [*]const f32 = @ptrCast(@alignCast(weight.data.ptr));
                tensor_ops.gemvF32(w_f32[0 .. weight.rows * weight.cols], in_x, bias, out_y, weight.rows, weight.cols);
            },
            else => {
                // Default fallback to F32
                const w_f32: [*]const f32 = @ptrCast(@alignCast(weight.data.ptr));
                tensor_ops.gemvF32(w_f32[0 .. weight.rows * weight.cols], in_x, bias, out_y, weight.rows, weight.cols);
            },
        }
    ...
    pub fn lookupEmbedding(self: *const TransformerEngine, token: u32, out_x: []f32) void {
        ...
        switch (self.tok_embeddings.storage_type) {
            ...
            .q8_0 => {
                ...
                const row_bytes = blocks_per_row * @sizeOf(quantization.BlockQ8_0);
                const row_slice = self.tok_embeddings.data[tid * row_bytes .. (tid + 1) * row_bytes];
                const blocks: [*]const quantization.BlockQ8_0 = @ptrCast(@alignCast(row_slice.ptr));
                for (0..blocks_per_row) |b| {
                    quantization.dequantizeBlockQ8_0(&blocks[b], 32, out_x[b * 32 .. (b + 1) * 32]);
                }
            },
            .q4_0 => {
                const blocks_per_row = dim / 32;
                const row_bytes = blocks_per_row * @sizeOf(quantization.BlockQ4_0);
                const row_slice = self.tok_embeddings.data[tid * row_bytes .. (tid + 1) * row_bytes];
                const blocks: [*]const quantization.BlockQ4_0 = @ptrCast(@alignCast(row_slice.ptr));
                for (0..blocks_per_row) |b| {
                    quantization.dequantizeBlockQ4_0(&blocks[b], 32, out_x[b * 32 .. (b + 1) * 32]);
                }
            },
            else => {
    ...
    pub fn forward(self: *TransformerEngine, token: u32, pos: usize) []const f32 {
        ...
        const n_rep = n_heads / n_kv_heads;
        ...
        for (self.layers.items, 0..) |layer, l| {
            ...
            @memcpy(k_cached, self.k);
            @memcpy(v_cached, self.v);
            ...
            for (0..n_heads) |h| {
                ...
                // Softmax over 0..=pos
                tensor_ops.softmaxF32(self.att[0 .. pos + 1], self.att[0 .. pos + 1]);
